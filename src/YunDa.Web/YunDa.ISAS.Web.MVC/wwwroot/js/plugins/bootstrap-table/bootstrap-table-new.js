                ...
                value: function initHeader() {
                    ...
                    this.options.columns.forEach(function (columns, i) {
                        ...
                        columns.forEach(function (column, j) {
                            ...
                            html.push("<th".concat(Utils.sprintf(' title="%s"', column.titleTooltip)), column.checkbox || column.radio ? '' : class_, Utils.sprintf(' style="%s"', halign + style), Utils.sprintf(' rowspan="%s"', column.rowspan), Utils.sprintf(' colspan="%s"', column.colspan), Utils.sprintf(' data-field="%s"', column.field), // If `column` is not the first element of `this.options.columns[0]`, then className 'data-not-first-th' should be added.
                            ...
                            if (column.checkbox) {
                                ...
                                if (!_this2.options.singleSelect && _this2.options.checkboxHeader) {
                                    text = '<div ' + ((column.checkbox || column.radio) && column['class'] ? ('class="' + column['class'] + '" style="padding-left:28px;margin-top:4px;margin-bottom:2px"') || '' : '') + '><input name="btSelectAll" type="checkbox" /><label></label></div>';
                                }
                ...
                value: function initRow(item, i, data, trFragments) {
                    ...
                    this.header.fields.forEach(function (field, j) {
                        ...
                        if (column.checkbox || column.radio) {
                            ...
                            text = [_this7.options.cardView ? "<div class=\"card-view ".concat(c, "\">") : "<td ".concat(style_, ">"), "<div " + (class_ ? (class_ + " style='padding-left:28px;margin-top:4px;margin-bottom:2px;'") : "") + "'>\n            <input\n            data-index=\"".concat(i, "\"\n            name=\"").concat(_this7.options.selectItemName, "\"\n            type=\"").concat(type, "\"\n            ").concat(Utils.sprintf('value="%s"', item[_this7.options.idField]), "\n            ").concat(Utils.sprintf('checked="%s"', isChecked ? 'checked' : undefined), "\n            ").concat(Utils.sprintf('disabled="%s"', isDisabled ? 'disabled' : undefined), " />\n            <label></label>\n            </div>"), _this7.header.formatters[j] && typeof value === 'string' ? value : '', _this7.options.cardView ? '</div>' : '</td>'].join('');
